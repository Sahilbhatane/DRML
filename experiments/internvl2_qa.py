...
from utils.utils import extract_html, extract_all_questions, remove_html_comments, extract_title, read_html, extract_text_from_html, cleanup_response, gpt_cost, load_internvl_image
from utils.screenshot import take_and_save_screenshot


"""
nlprun -m sphinx4 -g 1 -c 4 -r 80G -a sketch2code --output /sailhome/lansong/Sketch2Code/logs/internvl_qa_v0.4.txt 'python /sailhome/lansong/Sketch2Code/Sketch2Code/internvl2_qa.py' -p high
...
def generate(model, tokenizer, screenshot_path, sketch_path, source_html_path, out_dir, img_id):
    ...
    for _ in range(3):
        try:
            ...
            output_texts, _ = model.chat(tokenizer, sketch, direct_system_message + "\n\n" + direct_user_prompt, generation_config, history=None, return_history=True)
            ...
            while not html_response and num_questions < max_turns:
                turn += 1
                agent_resp, history = model.chat(tokenizer, agent_image, agent_message, generation_config, history=history, return_history=True)
                ...
                for i in range(len(questions)):
                    ...
                    output_texts, _ = model.chat(tokenizer, sketch, direct_system_message + "\n\n" + qa_user_prompt, generation_config, history=None, return_history=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_dir', type=str, default='/juice2/scr2/nlp/pix2code/zyanzhe/sketch2code_eval/v0.2/internvl_qa')
